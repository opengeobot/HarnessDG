...
public final class SessionCookies {
    ...
    public static void issue(HttpServletResponse response, String refreshToken, String csrfToken,
        ...
        response.addHeader("Set-Cookie", build(REFRESH_COOKIE, refreshToken, maxAgeSeconds, true, secure));
    ...
    public static void clear(HttpServletResponse response, boolean secure) {
        response.addHeader("Set-Cookie", build(REFRESH_COOKIE, "", 0, true, secure));
        response.addHeader("Set-Cookie", build(CSRF_COOKIE, "", 0, false, secure));
    }

    private static String build(String name, String value, long maxAge, boolean httpOnly, boolean secure) {
        StringBuilder sb = new StringBuilder(name).append('=').append(value)
                .append("; Path=/api/v1/auth; Max-Age=").append(maxAge)
                .append("; SameSite=Lax");
        if (httpOnly) {
            sb.append("; HttpOnly");
        }
