...
import com.modelhub.catalog.domain.RepositoryFavoriteEntity;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Modifying;
import org.springframework.data.jpa.repository.Query;
import org.springframework.data.repository.query.Param;

public interface RepositoryFavoriteRepository extends JpaRepository<RepositoryFavoriteEntity, Long> {

    boolean existsByUserIdAndRepositoryId(Long userId, Long repositoryId);

    /** 幂等插入：唯一约束冲突静默忽略（03 §6.1 并发兜底，不抛竞态异常）。 */
    @Modifying
    @Query(value = "INSERT INTO repository_favorites(user_id, repository_id, created_at) "
