...
scenarioEvidencePlan:
  - AC-P3-SAGA-001|E3|Injected failures for tag/PG windows with retry and reconciliation
crossCuttingPlan:
...
allowedPaths:
  - backend/src/main/java/com/aihub/version/infrastructure/PublishJobHandler.java
  - backend/src/main/java/com/aihub/integration/gitea/infrastructure/PublishedVersionReconciler.java
  - backend/src/test/java/com/aihub/version/infrastructure/PublishJobHandlerTest.java
  - docs/ai-spec/tasks/evidence/EVD-P3005-001.yaml
...
requiredValidationCommands:
  ...
  - pwsh ./docs/ai-spec/tools/validate-task-card.ps1 -TaskPath docs/ai-spec/tasks/TASK-P3-005.md
  - pwsh ./docs/ai-spec/tools/validate-task-card.ps1 -TaskPath docs/ai-spec/tasks/TASK-P3-005.md -CheckChangedPaths
  - ./mvnw -q -pl backend -Dtest=PublishJobHandlerTest test
  - ./mvnw -q -pl backend verify
approvedBy: User (plan execution authorization 2026-07-10)
approvedAt: 2026-07-10T00:00:00Z
---
# TASK-P3-005：发布 Saga 故障/超时/补偿

> 状态：`READY`

## 1. 目标

发布 Saga 各步骤故障可查询、重试与补偿，不盲删 Tag。

## 2. 关联规格

```yaml
requirements: ['REQ-PUB-001']
acceptanceScenarios: ['AC-P3-SAGA-001']
decisions: ['DEC-010']
```
...
| --- | --- | --- | --- | --- | --- |
| `AC-P3-SAGA-001` | Actor | Given preconditions | Execute action | PASS criteria | `E3` |
