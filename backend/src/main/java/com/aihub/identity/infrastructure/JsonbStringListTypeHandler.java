/*
 * 功能: MyBatis 类型处理器，在 List<String> 与 PostgreSQL jsonb 列之间互转（identity 模块）。
 * 时间: 2026-06-30
 * 作者: AxeXie
 */
 ...
 *
 * <p>写入时以 {@code Types.OTHER} 传入 JSON 字符串由 PostgreSQL 驱动转换为 jsonb；读取时解析为列表。
 * 用于 {@code scopes} / {@code tool_allowlist} 列。
 */
